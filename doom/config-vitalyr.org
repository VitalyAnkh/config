# SPDX-FileCopyrightText: © 2020-2022 VitalyR <vitalyankh@gmail.com>
...
Now I've discovered the wonders of literate programming, and am becoming more settled by the day. This is both my configuration, and a cautionary tale (just replace "Linux" with "Emacs" in the comic below).
...
;; Copyright (C) %Y TEC
;;
;; Author: VitalyR <https://git.tecosaur.net/tec>
;; Maintainer: VitalyR <contact@tecosaur.net>
;; Created: %B %d, %Y
;; Modified: %B %d, %Y
;; Version: %Y.%m.%d
...
(racket
 +lsp
 +xp
 +tree-sitter)            ; a DSL for DSLs
(raku
 +lsp
 +tree-sitter)                         ; the artist formerly known as perl6
...
Since we're using 10 [[Marginalia]] too, the separation between buffers and files is
...
(after! consult
  ...
  ;; Map the handy "C-z" shortcut
  (keymap-global-set "C-z" 'consult-complex-command)
  )
#+end_src
...
#+end_src
*** Flycheck
I don't like the default error checking. Let's tweak.
#+begin_src emacs-lisp
...
;; make org-babel use lower case keyword, avoid logseq's bug
...
;; used for avy-go-to-char-2
(keymap-set org-mode-map "C-'" 'avy-goto-char-2)
;; used for embark-act
