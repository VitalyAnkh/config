...
((org-dev (env ("DOOMDIR" . "~/.config/org-dev"))))
#+end_src
...
#+begin_src emacs-lisp :tangle ../org-dev/init.el :mkdirp yes
;;; init.el -*- lexical-binding: t; -*-
...
#+begin_src emacs-lisp :tangle ../org-dev/packages.el :noweb no-export
(unpin! org) ; there be bugs
#+end_src

#+begin_src emacs-lisp :tangle ../org-dev/config.el
(require 'org)
...
(setq doom-font (font-spec :family "JetBrains Mono" :size 14)
      ...
      doom-variable-pitch-font (font-spec :family "Linux Biolinum" :size 17)
      ;; doom-variable-pitch-font (font-spec :family "CMU Typewriter Text" :size 17)
      doom-serif-font (font-spec :family "Linux Biolinum" :size 17))
...
We could Drag text from emacs to other apps. Thanks to Po Lu!
...
#+begin_src emacs-lisp :tangle packages.el
(package! autocorrect :recipe (:host github :repo "tecosaur/autocorrect"))
#+end_src
...
#+begin_src emacs-lisp :tangle packages.el :noweb-ref none
(package! doom-modeline-media-player :recipe (:local-repo "lisp/doom-modeline-media-player"))
#+end_src
...
Mimick =evil='s ~/~ in =treemacs= buffer:
...
#+call: confpkg("!Pkg Smart-input-source")
...
Autoload =f90-mode= on =cuf= / =CUF= files.
#+begin_src emacs-lisp
(setq auto-mode-alist
      (append
       '(("\\.cuf\\'" . f90-mode)
...
#+end_src
** C/C++
#+call: confpkg("!Pkg clang-format")

=clang-format-plus= could format C/C++ code only in the modified region. Only formating modified code is a function long awaited, and [[https://github.com/microsoft/vscode/pull/104994][is provided by vscode three years ago]]. Hope I will implement this feature for any languages in the future.
...
Use =c++-mode= to edit =.cu= files. It's more suitable than =cuda-mode=.
#+begin_src emacs-lisp
(setq auto-mode-alist
      (append
       '(("\\.cu\\'" . c++-mode))
...
#+call: confpkg("!Pkg just-mode")

=just= is a command runner. It's a great tool to run commands like =Makefile=.
#+begin_src emacs-lisp :tangle packages.el
(package! just-mode
...
#+call: confpkg("!Pkg meson")

Meson is a great build tool for ~C~ and ~C++~, much more ergonomic than CMake.
#+begin_src emacs-lisp :tangle packages.el
(package! meson-mode)
#+end_src
...
#+end_src
** WebAssembly
#+call: confpkg("!Pkg WebAssembly")

Oh, post-modern VM runs everything anywhere!
#+begin_src emacs-lisp :tangle packages.el
...
For further control, some features may only be used when certain other features
