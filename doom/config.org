...
#+end_src
Thanks to Pu Lo, scrolling in emacs is great again!
#+begin_src emacs-lisp
...
I find the ~doom-solarized-light~ really nice. Oh, and with the nice selection doom provides there's no reason for me to want the defaults.
...
*** Meow
Modal editing on the wheel! Use =moew= for modal life.
#+begin_src emacs-lisp :tangle packages.el
(package! meow)
#+end_src
...
** Input Method
I can't go out without my favorite input method: [[https://fcitx-im.org/][fcitx5]], especially
within Emacs. Let's add =smart-input-source= to support fcitx5.
#+begin_src emacs-lisp :tangle packages.el
(package! sis)
#+end_src
