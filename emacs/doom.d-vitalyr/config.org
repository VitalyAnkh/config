...
- [[#general-configuration][General configuration]]
  ...
  - [[#enable-rainbow-mode-at-startup][Enable ~rainbow-mode~ at startup]]
...
- [[#org-mode][Org mode]]
  ...
  - [[#turn-on-org-footnote-auto-adjust][Turn on =org-footnote-auto-adjust=]]
  - [[#capturing-and-note-taking][Capturing and note taking]]
  - [[#org-for-academic][Org for Academic]]
    - [[#citar][=Citar=]]
    - [[#use-xenops-for-org-mode-latex-async-compilation][Use ~xenops~ for =org-mode= ~LaTeX~ async compilation]]
  ...
  - [[#org-roam][Org-roam]]
    - [[#use-org-transclusion][Use =org-transclusion=]]
  ...
  - [[#revealjs-presentations][Reveal.js presentations]]
    - [[#i-use-org-reveal-for-presentation][I use =org-reveal= for presentation.]]
  - [[#other-exporters][Other exporters]]
  - [[#programming-org][Programming Org]]
- [[#eaf][EAF]]
  - [[#install-eaf][install EAF]]
  - [[#config][Config]]
- [[#coding][Coding]]
  - [[#haskell][Haskell]]
  - [[#rust][Rust]]
...
- [[#useful-tools][Useful tools]]
  ...
  - [[#rgel][=rg.el=]]
    - [[#install-rgel][Install =rg.el=:]]
    - [[#enable-rgels-default-key-map-rg-menu-under-the-default-prefix-key-c-c-s][Enable =rg.el='s default key map =rg-menu= under the default prefix key =C-c s=.]]
  -  [[#wakatime][wakatime]]
- [[#experiments][Experiments]]
  - [[#emacs-rust-dynamic-module-experiments][Emacs Rust dynamic module experiments]]
...
#+end_src
Set ~ivy-use-selectable-prompt~ (needless after using =vertico= ):
#+begin_src emacs-lisp
...
** Enable ~rainbow-mode~ at startup
#+begin_src emacs-lisp
(rainbow-mode)
#+end_src
...
Set ~word-wrap-by-category~ to make Chinese and English characters align on the same line:
#+begin_src emacs-lisp
...
(let* ((variable-tuple
        (cond
         ...
         ((x-list-fonts "DejaVu Math TeX Gyre")         '(:font "DejaVu Math TeX Gyre"))
...
#+end_src
** Turn on =org-footnote-auto-adjust=
#+begin_src emacs-lisp
...
#+end_src
*** Use ~xenops~ for =org-mode= ~LaTeX~ async compilation
#+begin_src emacs-lisp :tangle packages.el
(package! xenops)
#+end_src
...
** Org-roam
*** Use =org-transclusion=
Transclusion is the ability to include content from one file into another by reference. Org-transclusion is an Org Mode version of it.
It lets you insert a copy of text content via a file link or ID link within an Org file.
#+begin_src emacs-lisp :tangle packages.el
...
I use =org-reveal= to make presentations. The functions below help me improve my workflow by automatically exporting the slides whenever I save the file, refreshing the presentation in my browser, and moving it to the slide where the cursor was when I saved the file. This helps keeping a "live" rendering of the presentation next to my Emacs window.
...
*** I use =org-reveal= for presentation.
#+begin_src emacs-lisp :tangle packages.el
...
#+begin_src emacs-lisp :tangle no
(use-package eaf
  :load-path "~/sdk/app/emacs-application-framwork" ; Set to "/usr/share/emacs/site-lisp/eaf" if installed from AUR
  :custom
...
** biblio
Use =citar= and =org-cite= to insert citation:
#+begin_src emacs-lisp
...
** Artist Mode
Disable line numbers when using ~artist-mode~:
#+begin_src emacs-lisp
(add-hook 'artist-mode-hook (lambda () (display-line-numbers-mode -1)))
#+end_src
** =rg.el=
*** Install =rg.el=:
#+begin_src eamcs-lisp :tangle packages.el
(package! rg)
#+end_src
*** Enable =rg.el='s default key map =rg-menu= under the default prefix key =C-c s=.
#+begin_src emacs-lisp
...
#+end_src
** Emacs Rust dynamic module experiments
#+begin_src emacs-lisp
